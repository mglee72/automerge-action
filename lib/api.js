...
async function handlePullRequestUpdate(context, eventName, event) {
  const { mergeRequireStatuses } = context;
  ...
  if (mergeRequireStatuses) {
    logger.debug("Fetching checks since checks are required");
    await fetchChecks(
      context,
      event.pull_request.base.repo,
  ...
  await merge(context, pullRequest, checks);
}
...
async function fetchChecks(context, repository, sha) {
  ...
  // let { data: statuses } = await octokit.repos.listCommitStatusesForRef({
  // });
