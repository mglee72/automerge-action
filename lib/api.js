...
async function handlePullRequestUpdate(context, eventName, event) {
  ...
  await merge(context, pullRequest, statuses);
}
...
async function fetchStatuses(context, repository, sha) {
  const { octokit } = context;

  logger.debug("Getting statuses info for", sha, "...");
  // let { data: statuses } = await octokit.repos.listCommitStatusesForRef({
  // });
  let response = await octokit.request('GET /repos/{owner}/{repo}/commits/{ref}/check-suites', {
    owner: repository.owner.login,
    repo: repository.name,
    ref: sha,
    mediaType: {
  ...
  logger.trace("Full statuses:", response);
  return response;
}
