...
function createConfig(env = {}) {
  ...
  function splitOrEmpty(str) {
    return (str == null ? [] : str.split(",").map(s => s.trim()));
  }
  ...
  const mergeRemoveLabels = splitOrEmpty(env.MERGE_REMOVE_LABELS);
  const mergeRequireStatuses = splitOrEmpty(env.MERGE_REQUIRE_STATUSES);
  const mergeRequireApproval = env.MERGE_REQUIRE_APPROVAL === "true";
  ...
  const failOnMergeSkip = env.FAIL_ON_MERGE_SKIP === "true"
  ...
  return {
    mergeLabels,
    mergeRemoveLabels,
    mergeRequireStatuses,
    mergeRequireApproval,
    mergeMethod,
    mergeForks,
    mergeCommitMessage,
