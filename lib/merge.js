...
async function skipPullRequest(context, pullRequest) {
  ...
  if (mergeRequireStatuses) {
    const checks = await fetchChecks(context, pullRequest.base.repo.name, pullRequest.head.sha);
    for (const required of mergeRequireStatuses) {
      let foundStatus = false;
      for (const status of checks) {
        logger.info("Found status:", status.name);
        logger.info("Found status1:", status);
        if (status.name == required) {
          foundStatus = true;
          if (status.status != 'success') {
            logger.info("Skipping PR merge, required check", required, "has state", status.status);
            skip = true;
          }
        }
      }
  ...
  if (mergeRequireApproval) {
    const approvals = await fetchApprovals(context, pullRequest.base.repo.name, pullRequest);
    let numApprovals = 0;
    ...
    if (numApprovals == 0) {
      logger.info("Skipping PR merge, require approval");
      skip = true;
    } else if (numApprovals < approvals.length) {
      logger.info("Skipping PR merge, not all approvals complete");
      skip = true;
    }
  }
...
async function fetchChecks(context, repository, sha) {
  const { octokit } = context;

  logger.debug("Getting check runs for", sha, "...");
...
async function fetchReviews(context, repository, pullRequest) {
  const { octokit } = context;
