...
async function merge(context, pullRequest, statuses) {
  if (skipPullRequest(context, pullRequest, statuses)) {
    return false;
  }
...
function skipPullRequest(context, pullRequest, statuses) {
  const {
    config: { mergeForks, mergeLabels, mergeRequireStatuses }
  } = context;
  ...
  for (const required of mergeRequireStatuses) {
    ...
    for (const status in statuses) {
      logger.info("Found status:", status.context);
      logger.info("Found status1:", status);
      if (status.context == required) {
        foundStatus = true;
        if (status.state != 'success') {
          logger.info("Skipping PR merge, required check", required, "has state", status.state);
          skip = true;
        }
      }
    }
